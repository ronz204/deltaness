...
`specifier` and `sentinel` both assume a knowledge base already exists for the slice they're working on. Day one of a project, it doesn't — there's no orientation rule to load a session, no `.cursor/docs/*` to check a spec against, nothing to read before writing. Surveyor exists to close that gap fast: one guided pass that turns a bare repo (or a repo with no knowledge base yet) into a working `.cursor/rules/orientation.mdc` plus whatever `.cursor/docs/*` categories the project actually warrants, instead of that context trickling in one ad hoc `archivist` call at a time over the first several sessions. Like its siblings, this skill is portable — nothing below assumes a specific stack or repo layout.
...
- Check whether `.cursor/rules/orientation.mdc` (or any `alwaysApply: true` rule serving that router role) already exists and has real content. If it does, this isn't a bootstrap — hand off to `archivist` for a normal, targeted update instead of re-running initialization over curated content.
...
- **Don't invent a project's internal directory layout beyond what this step actually found on disk, or what Step 2 gets the user to state explicitly.** This is the step most likely to produce a rule candidate whose natural `globs` scoping wants a subfolder split (e.g. domain vs. application vs. infrastructure) that hasn't actually been created or decided yet — a scaffolded skeleton isn't a decided layout, and neither is a plausible-looking convention borrowed from how similar projects are usually structured. When that happens, either scope the candidate to the nearest boundary that genuinely is settled (e.g. a named service directory already recorded in the orientation rule's repo layout) or ask the user directly in Step 2 rather than guessing — a rule scoped to a folder that doesn't exist yet silently never fires, which is worse than not having the rule.
...
- **Infrastructure** — whether the project has static infrastructure/provisioning facts (hosting, environments, provisioned resources) worth capturing in `.cursor/docs/structure.md`'s topology/cross-cutting content, if Step 1 couldn't tell from config alone.
...
- `.cursor/rules/orientation.mdc` via `references/mark.template.md` — project description, knowledge base layout, repo layout, setup/common commands, conventions.
- `overview.md`, `modules.md`, `structure.md`, `database.md`, `expertise.md`, and `approach.md` — these already exist as shipped skeletons under `.cursor/docs/`; fill each in place with what Step 1/Step 2 found rather than creating a new file from `references/docs.template.md`. State "not applicable here" plainly in one of these rather than leaving it as an unfilled skeleton, on the rare project where it genuinely doesn't apply.
