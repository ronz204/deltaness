...
- **Keep expensive reading out of the main session where possible.** If confirming a claim needs broader reading/judgment (not just a grep hit), and this environment offers a background or parallel agent/task mechanism, dispatch the read to it per slice (or per cluster of related claims) so only the verdict comes back — run independent slices' checks in parallel rather than sequentially. Cursor has no file-based, user-defined subagent feature, so there's no dedicated per-check worker to reuse or create here — if no delegation mechanism is available in this session, do the targeted read directly but keep it scoped to exactly what the claim needs, never the whole slice's surrounding code.
...
**How it escalates.** Run the check scoped to that single claim, never to the whole slice, using only Read, Glob, Grep, and Bash (via a background/parallel task where this environment offers one, otherwise directly in-session) — there's no file-based subagent to reuse for this.
